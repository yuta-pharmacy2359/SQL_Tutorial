SELECT * FROM Shohin;
/*
 shohin_id |  shohin_mei  | shohin_bunrui | hanbai_tanka | shiire_tanka |  torokubi  | shohin_mei_kana 
-----------+--------------+---------------+--------------+--------------+------------+-----------------
 0002      | 穴あけパンチ   | 事務用品       |          500 |          320 | 2009-10-10 | 
 0008      | ボールペン     | 事務用品       |          100 |              |            | 
 0006      | フォーク       | キッチン用品   |         5000 |              | 2009-10-10 | 
 0007      | おろし金       | キッチン用品   |         8800 |          790 | 2009-10-10 | 
 0004      | 包丁           | キッチン用品  |        29000 |         2800 | 2009-10-10 | 
 0001      | Tシャツ        | 衣服         |         2500 |          500 | 2009-10-10 | 
(6 rows)
*/

INSERT INTO Shohin SELECT * FROM Shohin;
/*
ERROR:  duplicate key value violates unique constraint "shohin_pkey"
DETAIL:  Key (shohin_id)=(0002) already exists.
*/
